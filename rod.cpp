...
/*======================================================================================================================
  ...
  The free computer code for calculating the contribution of the primary longitudinal plasmonic resonance
    of a cylinder with round edges to the extinction and scattering cross sections using
...
/***********************************************************************************************************************
  The analytical model for a cylinder with round edges.
***********************************************************************************************************************/

/*----------------------------------------------------------------------------------------------------------------------
  Volume of a cylinder with round edges.
----------------------------------------------------------------------------------------------------------------------*/
double volume(
  const double &L,                          // Length in nm.
  const double &D,                          // Diameter in nm.
  const double &R)                          // Edge radius in nm.
{
  double tor_rad = 0.5*D - R;
  double outer_half_toroid = 2.0*M_PI*(tor_rad + 4.0*R/(3.0*M_PI) )*M_PI*R*R*0.5; // Pappus' second centroid theorem
  return 0.25*M_PI*D*D*(L - 2.0*R) + 2.0*M_PI*tor_rad*tor_rad*R + outer_half_toroid;
}
  ...
  const double &R)                          // Edge radius in nm.
{
  ...
  double beta = (0.353865*pow(D/L, -0.220737) + 0.41313) + (-0.00516255*pow(D/L, 0.0942578) + 0.00313908)*R/D;
  double eps_c = (-1.22129*pow(D/L, -1.64564) - 4.10255) + (1.44845*pow(D/L, -0.59064) + 1.6919)*R/D;
  double a2 = (1.84051*pow(D/L, 0.818259) - 0.164418) + (0.231667*pow(D/L, 0.398876) - 0.234)*R/D;
  double a4 = (0.36635*pow(D/L, 0.12432) + 0.349012) + (0.179266*pow(D/L, 0.106472) + 0.171246)*R/D;

  double s = std::sqrt(eps_h)*L/lambda;
  double V1 = beta*volume(L, D, R);
...
double scatCSdip(
  ...
  const double &R)                          // Edge radius in nm.
{
  std::complex<double> polariz = dipPolariz( lambda, eps_m, eps_h, L, D, R );
...
/*----------------------------------------------------------------------------------------------------------------------
  Diameter of a sphere of the same volume as a cylinder with round edges.
----------------------------------------------------------------------------------------------------------------------*/
double diameter(
  const double &L,                          // Length in nm.
  const double &D,                          // Diameter in nm.
  const double &R)                          // Edge radius in nm.
{
  return 2.0*pow( 3.0*volume(L, D, R)/(4.0*M_PI), 1.0/3.0 );
}
...
int main(int argc, char **argv)
{
  ...
  const double L_size =  50.0;              // Length in nm.
  const double D_size =  10.0;              // Diameter in nm.
  const double R_size =   3.0;              // Edge radius in nm.

  const double eps_h = 1.78;                // Dielectric permittivity of host media.
  ...
  const double wl_max = 2000.0;             // Maximal value of wavelength range to print results in nm.
  ...
  const double D_SD = diameter(L_size, D_size, R_size);
  ...
  for (int i = 0; i < wl_n; ++i) {
    ...
    std::complex<double> val = dipPolariz(wl, eps_m, eps_h, L_size, D_size, R_size);
    fout_re << wl << " " << std::real(val) << std::endl;
    fout_im << wl << " " << std::imag(val) << std::endl;
    fout_sc << wl << " " << scatCSdip(wl, eps_m, eps_h, L_size, D_size, R_size) << std::endl;
    fout_ex << wl << " " << extCSdip(wl, eps_m, eps_h, L_size, D_size, R_size) << std::endl;
  }
  fout_re.close();
  fout_im.close();
  fout_sc.close();
